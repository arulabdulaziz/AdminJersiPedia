...
const JerseyEdit = (props) => {
  ...
  const [loadingRender2, setLoadingRender2] = useState(false);
  ...
  const getDetaiJersey = () => {
    setLoadingRender2(true);
    FIREBASE.database()
      .ref("jerseys")
      ...
      .once("value", (querySnapShot) => {
        ...
        if (result) {
          ...
          setSizeSelected(result.size);
          setType(result.type);
          setWeight(+result.weight);
        }
        setLoadingRender2(false);
      })
      .catch((error) => {
...
export default connect(mapStateToProps, mapStateToDispatch)(JerseyEdit);
